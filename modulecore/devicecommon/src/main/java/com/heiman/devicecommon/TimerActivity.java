...
public class TimerActivity extends BaseActivity implements View.OnClickListener, CompoundButton.OnCheckedChangeListener{
    ...
    private int tempWeekFalg;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentLayout(R.layout.devicecommon_activity_timer);
    ...
    protected void onResume() {
        ...
        String time = date.getHours() + ":" + (date.getMinutes() < 10 ? "0" + date.getMinutes() : date.getMinutes() + "" );
    ...
    public void onClick(View v) {
        if (!UsefullUtill.judgeClick(R.layout.devicecommon_activity_timer, 500)) {
            BaseApplication.getLogger().e("点击过快！");
            return;
        }

        if (v.getId() == R.id.title_tv_Right) {
            LogUtil.e("点击提交！");
            return;
        }

        if (v.getId() == R.id.rl_repeat_container) {
            LogUtil.e("点击选择重复！");
            showSetLinkRepeatDialog();
            return;
        }

        if (v.getId() == R.id.rl_start_time_container) {
            LogUtil.e("点击选择开启时间！");
            setStartTime();
            return;
        }

        if (v.getId() == R.id.txt_one_time) {
            dismissDialog(v);
            LogUtil.e("选择只执行一次!");
            weekFalg = 0;
            txtRepateTimes.setText(R.string.one_time);
        ...
        if (v.getId() == R.id.txt_everyday) {
            dismissDialog(v);
            LogUtil.e("选择每天执行！");
            weekFalg = SmartHomeUtils.Everyday;
            txtRepateTimes.setText(R.string.everyday);
            return;
        }

        if (v.getId() == R.id.txt_monday_to_friday) {
            dismissDialog(v);
            LogUtil.e("选择周一至周五执行！");
            weekFalg = 0x1f;
    ...
    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
        ...
        if (buttonView.getId() == R.id.cbx_monday) {
            if (isChecked) {
                tempWeekFalg = tempWeekFalg + SmartHomeUtils.Monday;
            } else {
                tempWeekFalg = tempWeekFalg - SmartHomeUtils.Monday;
            }
            return;
        ...
        if (buttonView.getId() == R.id.cbx_tuesday) {
            if (isChecked) {
                tempWeekFalg = tempWeekFalg + SmartHomeUtils.Tuesday;
            } else {
                tempWeekFalg = tempWeekFalg - SmartHomeUtils.Tuesday;
            }
            return;
        ...
        if (buttonView.getId() == R.id.cbx_wednesday) {
            if (isChecked) {
                tempWeekFalg = tempWeekFalg + SmartHomeUtils.Wednesday;
            } else {
                tempWeekFalg = tempWeekFalg - SmartHomeUtils.Wednesday;
            }
            return;
        ...
        if (buttonView.getId() == R.id.cbx_thursday) {
            if (isChecked) {
                tempWeekFalg = tempWeekFalg + SmartHomeUtils.Thursday;
            } else {
                tempWeekFalg = tempWeekFalg - SmartHomeUtils.Thursday;
            }
            return;
        ...
        if (buttonView.getId() == R.id.cbx_friday) {
            if (isChecked) {
                tempWeekFalg = tempWeekFalg + SmartHomeUtils.Friday;
            } else {
                tempWeekFalg = tempWeekFalg - SmartHomeUtils.Friday;
            }
            return;
        ...
        if (buttonView.getId() == R.id.cbx_saturday) {
            if (isChecked) {
                tempWeekFalg = tempWeekFalg + SmartHomeUtils.Saturday;
            } else {
                tempWeekFalg = tempWeekFalg - SmartHomeUtils.Saturday;
            }
            return;
        ...
        if (buttonView.getId() == R.id.cbx_sunday) {
            if (isChecked) {
                tempWeekFalg = tempWeekFalg + SmartHomeUtils.Sunday;
            } else {
                tempWeekFalg = tempWeekFalg - SmartHomeUtils.Sunday;
            }
            return;
    ...
    private void showSetLinkRepeatDialog() {
        View view = LayoutInflater.from(this).inflate(R.layout.devicecommon_layout_set_link_repeat_dialog, null);
    ...
    private void showSetCustomLinkRepeatDialog() {
        View view = LayoutInflater.from(this).inflate(R.layout.devicecommon_layout_set_link_custom_repeat_dialog, null);
        Dialog dialog = new Dialog(this, R.style.dialog_set_repeat);
        dialog.setContentView(view);
        dialog.setCancelable(true);
        dialog.setCanceledOnTouchOutside(false);
        ...
        while (i < 0x80) {
            int s = weekFalg & i;
            switch (s) {
                case SmartHomeUtils.Monday:
                    cbxMonday.setChecked(true);
                    break;
                case SmartHomeUtils.Tuesday:
                    cbxTuesday.setChecked(true);
                    break;
                case SmartHomeUtils.Wednesday:
                    cbxWednesday.setChecked(true);
                    break;
                case SmartHomeUtils.Thursday:
                    cbxThursday.setChecked(true);
                    break;
                case SmartHomeUtils.Friday:
                    cbxFriday.setChecked(true);
                    break;
                case SmartHomeUtils.Saturday:
                    cbxSaturday.setChecked(true);
                    break;
                case SmartHomeUtils.Sunday:
