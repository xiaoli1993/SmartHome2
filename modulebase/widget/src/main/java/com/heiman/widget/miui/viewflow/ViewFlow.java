 ...
 */
package com.heiman.widget.miui.viewflow;
...
//		Log.e("ViewFlow", ev.toString());
	...
	public void setSelection(int position) {
		...
		position = Math.min(position, (mAdapter.getCount() > 0 ? (mAdapter.getCount() - 1) : 0));
		recycleViews();
	...
	private void postViewSwitched(int direction) {
		...
		if (mIndicator != null && mCurrentBufferIndex >= 0 && mCurrentBufferIndex < mLoadedViews.size()) {
			mIndicator.onSwitched(mLoadedViews.get(mCurrentBufferIndex),
					mCurrentAdapterIndex);
		}
		if (mViewSwitchListener != null && mCurrentBufferIndex > 0 && mCurrentBufferIndex < mLoadedViews.size()) {
			mViewSwitchListener
